...
namespace DeleteUserPlant
{
    ...
    public class Function
    {
        ...
        public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest apigProxyEvent, ILambdaContext context)
        {
            ...
            try {
                if (dbCon.IsConnect())
                {
                    string query = "USE plant_care_app;" +
                        "DELETE FROM tbl_user_plant WHERE user_plant_id = '" + userPlantId +  "';"; 
                    var cmd = new MySqlCommand(query, dbCon.Connection);
                    // cmd.Parameters.AddWithValue("@user_plant_id", userPlantId);
                    // cmd.Prepare();
                    cmd.ExecuteNonQuery();
                }
            } finally {
            ...
            return new APIGatewayProxyResponse
            {
                Body = JsonSerializer.Serialize("Sucessfully deleted plant with userPlantId: " + userPlantId),
                StatusCode = 200,
                Headers = new Dictionary<string, string> { 
                    { "Content-Type", "application/json" },
                    { "Access-Control-Allow-Headers", "Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token" },
